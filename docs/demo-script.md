...
adds its API key and OpenAPI spec, a buyer pays through Paykubo, and the Video
Launch Campaign Agent later buys the same tool from a funded budget. Each paid
action settles in USDC on Morph and creates receipts, provider traces, and
proof-ready records.
...
- **Show:** Paykubo homepage with Morph, USDC, x402, marketplace, and agent
  messaging.
- **Voiceover:** Paykubo is our Build In! Payments project for the x402
  Agentic Payments track. It turns existing APIs, like video generation, into
  USDC-paid tools on Morph that people and agents can buy per request through
  x402.

### 0:20-0:45 - Add ClipLore

- **Show:** cliplore.ai API key page, copied OpenAPI URL, then Paykubo provider
  product form.
- **Voiceover:** I generate a ClipLore API key, copy the OpenAPI URL, import it
  into Paykubo, choose the video job endpoint, add the key server-side, price it
  in USDC, and publish it as agent-ready.

### 0:45-1:10 - Run From Marketplace

- **Show:** `/marketplace`, ClipLore product, `/orders/new`, payment console,
  completed order, and result link.
- **Voiceover:** A buyer opens the marketplace, runs the ClipLore video tool,
  and pays through x402. Paykubo settles USDC on Morph before starting the
  provider job, then returns the ClipLore output link.
...
- Open `https://cliplore.ai/auth/sign-in?next=/dashboard/developers/api-keys`.
- Sign in.
- Create a live API key named `Paykubo marketplace`.
- Copy the API key.
- Open `https://cliplore.ai/developers/openapi`.
- Copy the OpenAPI URL.
- **Voiceover:** ClipLore already has the video API. I only need its API key and
  OpenAPI URL to publish it as a paid Paykubo tool.

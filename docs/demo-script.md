...
- **Move to:** one product detail page, then `/orders/new` or an existing
  `/orders/[orderId]`
...
- **Move to:** `/proofs/[proofId]`
- **Show:** Public proof, run metadata, receipts, proof hash, attestation link.
- **Say:**
  > "The public proof page makes the run auditable without exposing private API
...
- **Voiceover:**
  ...
  > goal is to make Morph the settlement layer for APIs that people and agents
  > can buy instantly, one request at a time."
...
> Morph.
>
> Each call creates a receipt with buyer, provider, amount, fee split, product,
...
> Next, we will add more Philippine and Southeast Asian providers, stronger
> agent templates for local business workflows, and production analytics. The
> goal is to make Morph the settlement layer for APIs that people and agents can
> buy instantly, one request at a time."
...
- A receipt screen shows buyer, provider, amount, fee split, and Morph
  transaction link.
- An agent run screen shows objective, budget, selected tools, paid actions, and
  receipts.
- A proof page shows public run metadata, receipt links, proof hash, and Morph
  attestation data.

## Pacing Notes

- Do not show provider onboarding, OpenAPI import setup, wallet switching, or
  external service dashboards in the 2-minute submission video.
- Use pre-created listings, orders, receipts, agent runs, and proofs so the
  recording stays focused on the product value.
- Keep the cursor moving only when it helps judges identify the current feature.
- If a live wallet prompt is slow, use an already completed order and explain
  the payment step while showing the receipt and Morph transaction link.
